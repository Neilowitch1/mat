...
export default function InventoryItemRow({
  ...
  async function handleAddToShoppingList() {
    ...
    try {
      ...
      setErrorMessage("Kunde inte lägga till i inköpslistan.");
    }
  }
  ...
  const shoppingActionLabel = {
    ...
    "already-exists": "Finns redan i inköpslistan",
  }[shoppingActionState];

  const LocationIcon = inventoryLocationIcons[item.location];
    ...
    <AppCard className="p-3.5">
      ...
      <div className="mt-1.5 flex items-center gap-2">
        <div className="flex shrink-0 items-center rounded-full border border-border bg-card p-0.5 shadow-[0_2px_8px_rgba(57,62,55,0.035)]">
          <Button type="button" variant="ghost" size="icon" disabled={isUpdating || item.quantity <= 0} onClick={() => handleQuantityChange(-1)} aria-label={`Minska mängden ${item.product?.name ?? "produkt"}`} className="size-[34px] rounded-full text-muted-foreground hover:bg-secondary">
            <Minus aria-hidden="true" className="size-3.5" />
          </Button>
          <span className="min-w-14 text-center text-sm font-medium tabular-nums text-foreground">
            {item.quantity} {item.unit}
          </span>
          <Button type="button" variant="ghost" size="icon" disabled={isUpdating} onClick={() => handleQuantityChange(1)} aria-label={`Öka mängden ${item.product?.name ?? "produkt"}`} className="size-[34px] rounded-full text-muted-foreground hover:bg-secondary">
            <Plus aria-hidden="true" className="size-3.5" />
          </Button>
        </div>
      </div>

      <fieldset className="mt-1.5 border-t border-border/70 pt-1.5">
        ...
        <div className="-mx-0.5 flex gap-0.5 overflow-x-auto rounded-full bg-secondary/60 p-0.5">
          {inventoryStatuses.map((option) => (
            <Button
              ...
              className={`h-[30px] shrink-0 rounded-full px-2.5 text-xs font-medium shadow-none ${
      ...
      <div className="mt-1 flex min-w-0 items-center justify-between gap-2 border-t border-border/60 pt-1">
      ...
      {(item.status === "half" ||
        ...
        <Button
          ...
          className="mt-1.5 h-9 rounded-full border-border bg-transparent px-3 text-muted-foreground hover:bg-secondary hover:text-foreground"
        >
