...
> = {
  expired: {
    className:
      "bg-[#f5e8e6] text-destructive hover:bg-[#efddda]",
    label: "Utgånget",
  },
  ...
  thisWeek: {
    className:
      "bg-[#f4eedf] text-[#8a7447] hover:bg-[#eee5cf]",
...
export default function InventoryExpirationControl({
  ...
  async function saveExpiration(
    ...
    try {
      const updatedItem =
        await updateInventoryExpiration(
          item.id,
          expiresAt
        );
      ...
      setErrorMessage(
        "Kunde inte ändra bäst före-datumet."
      );
    } finally {
      setIsUpdating(false);
    }
